...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/agent-stack-cli-');
    const execPath = path.join(root, '.claude/skills/test-stack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/test-stack/browse/src/server.ts');
