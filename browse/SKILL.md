...
version: 0.0.1
description: |
  Fast headless browser for QA testing, deployment checks, screenshots, and
  live demos. Supports Chromium by default and can launch Chrome, Edge, or
  Firefox via BROWSE_BROWSER.
allowed-tools:
  - Bash
  - Read
...
# browse: QA Testing, Smoke Checks, and Demo Verification

Persistent Playwright browser session. First call auto-starts, then later calls reuse the same browser state.

## Browser selection

Set `BROWSE_BROWSER` before launching the session:

```bash
BROWSE_BROWSER=chromium browse goto https://example.com
BROWSE_BROWSER=chrome browse goto https://example.com
BROWSE_BROWSER=edge browse goto https://example.com
BROWSE_BROWSER=firefox browse goto https://example.com
```

Cookie import from a real browser currently supports Chromium-family browsers only.
