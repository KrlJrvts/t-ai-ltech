...
import { chromium, firefox, type Browser, type BrowserContext, type Page, type Locator } from 'playwright';
import { addConsoleEntry, addNetworkEntry, addDialogEntry, networkBuffer, type DialogEntry } from './buffers';

type SupportedBrowser = 'chromium' | 'chrome' | 'edge' | 'firefox';

function normalizeBrowseBrowser(raw: string | undefined): SupportedBrowser {
  const normalized = (raw || 'chromium').toLowerCase();
  switch (normalized) {
    case 'chromium':
    case 'chrome':
    case 'edge':
...
export class BrowserManager {
  ...
  private readonly requestedBrowser: SupportedBrowser = normalizeBrowseBrowser(process.env.BROWSE_BROWSER);
  ...
  async launch() {
    this.browser = await this.launchRequestedBrowser();

    // Browser crash → exit with clear message
    this.browser.on('disconnected', () => {
      console.error(`[browse] FATAL: ${this.requestedBrowser} process crashed or was killed. Server exiting.`);
  ...
  private async launchRequestedBrowser(): Promise<Browser> {
    switch (this.requestedBrowser) {
      case 'firefox':
