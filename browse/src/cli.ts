...
export function resolveServerScript(
  ...
  // Fallback: search user-level skills installs dynamically
  const skillsDir = path.resolve(env.HOME || '/tmp', '.claude', 'skills');
  try {
    const entries = fs.readdirSync(skillsDir, { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;
      const candidate = path.resolve(skillsDir, entry.name, 'browse', 'src', 'server.ts');
  ...
  return path.resolve(env.HOME || '/tmp', '.claude', 'skills', 'browse', 'src', 'server.ts');
}
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`browse — Fast headless browser for coding agents
